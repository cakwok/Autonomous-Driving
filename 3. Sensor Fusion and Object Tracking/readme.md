...
Extended Kalman Filter achieves so by computing a non-linear measurement update by linear approximation functions, in this case the first order Taylor expansion.  Since we have multivariates of px, py, pz locations, we use Jacobian as the measurement projection function.
