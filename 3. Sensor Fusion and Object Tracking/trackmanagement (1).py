...
class Track:
    ...
    def __init__(self, meas, id):
        ...
        pos_veh = meas.sensor.sens_to_veh * pos_sens # self.sens_to_veh = np.matrix(calib.extrinsic.transform).reshape(4,4) 
        ...
        for i in unassigned_tracks:
            ...
            if meas_list: # if not empty
                if meas_list[0].sensor.in_fov(track.x):
                    # gradually decrease the confidence in a track over time if it has not been associated with 
                    # any measurements for a while
