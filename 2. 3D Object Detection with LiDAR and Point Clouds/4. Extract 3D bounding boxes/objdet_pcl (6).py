...
def show_pcl(pcl):
    ...
    visualize_pcl = o3d.visualization.VisualizerWithKeyCallback()
    visualize_pcl.create_window()
    #visualize_pcl.get_render_option().background_color = np.asarray([0, 0, 0])
    ...
    visualize_pcl.add_geometry(pcd)
...
def show_range_image(frame, lidar_name):
    ...
    #ri_intensity = np.amax(ri_intensity)/2 * ri_intensity * 255 / (p99 - p1) 
    ri_intensity = (ri_intensity-p1) * 200/(p99 - p1) 
    ri_intensity[ri_intensity > 255] = 255
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    print("lidar_pcl", lidar_pcl[:100])
    print("lidar_pcl_cpy", lidar_pcl_cpy[:100, 0])

    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    lidar_pcl_cpy[:, 1] = np.int_(np.floor(lidar_pcl_cpy[:, 1] / bev_discret) + (configs.bev_width + 1) / 2)
    print("lidar_pcl_cpy", lidar_pcl_cpy[:100, 1])
    print("lidar_pcl_cpy", lidar_pcl_cpy)
    ...
    idx_height = np.lexsort((-lidar_pcl_cpy[:, 2], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]), axis=0)
    ...
    intensity_map[np.int_(lidar_pcl_int[:, 0]), np.int_(lidar_pcl_int[:, 1])] = lidar_pcl_int[:, 3] / (np.amax(lidar_pcl_top[:, 3])-np.amin(lidar_pcl_top[:, 3]))

    ## step 5 : temporarily visualize the intensity map using OpenCV to make sure that vehicles separate well from the background
    img_intensity = intensity_map * 255 * 4 
