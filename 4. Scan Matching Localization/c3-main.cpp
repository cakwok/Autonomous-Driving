	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
            ...
            pose = Pose(Point(vehicle->GetTransform().location.x, vehicle->GetTransform().location.y
            				  ...
            				  ,vehicle->GetTransform().rotation.pitch * pi/180, vehicle->GetTransform().rotation.roll * pi/180)) - poseRef;
            
			// TODO: Find pose transform by using ICP or NDT matching
			//pose = ....
            Eigen::Matrix4d pose_transform = ICP(mapCloud, cloudFiltered, pose, 0);
