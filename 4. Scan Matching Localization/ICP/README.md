...
The resulting output showcases a map represented by big square with blue dots, depicting the LiDAR signal after the first scan at starting pose. After moving by an unknown distance and angle, the next LiDAR scan is represented by red dots. Our goal is to estimate the new location of the robot using the source LiDAR cloud points derived from the red dots.
